            ...
            $(options.selectSubmit).click(function () {
                Do.ready('tips', 'dialog', function () {
                    ...
                    var dialog = layer.confirm('你确认要进行本次批量操作！', function () {
                        var type=$(options.selectAction).val();
                        if(type==2){
                        	var oppassword=$('#oppassword').val();                      	 
                        	var txt=  "请输入操作密码";
					        var _this=$(this);
                            layer.close(dialog);//关闭遮罩
                            ...
                            window.wxc.xcConfirm(txt, window.wxc.xcConfirm.typeEnum.input,{
                            	...
                            	toastr.warning("操作密码验证错误");
                            }
                        	  }
                            })
                        }else if(type==1){
                        	//如果是管理员级别的人操作删除需要进行操作密码验证
                        	var oppassword=$('#oppassword').val();                      	 
                        	var txt=  "请输入操作密码";
            ...
            function operat(obj,url,success,failure){
                Do.ready('tips', 'dialog', function () {
                    ...
                    var dialog = layer.confirm('你确认执行' + text + '操作？', function () {
                        ...
                        var allow=$(obj).attr('confirm');                      
                        if(allow ==1){
                        	//如果是管理员级别的人操作删除需要进行操作密码验证
                        	var oppassword=$('#oppassword').val();                      	 
                        	var txt=  "请输入操作密码";
					        var _this=$(this);
					        layer.close(Dload); //关闭遮罩
                            layer.close(dialog);
                        	window.wxc.xcConfirm(txt, window.wxc.xcConfirm.typeEnum.input,{
						    onOk:function(v){
						    remark=v;
